...
void LockerWindow::HandleEvents(sf::Event& event) {
    ...
    if (event.type == sf::Event::MouseButtonPressed) {
        if (event.mouseButton.button == sf::Mouse::Left) {
                ...
                if (i < entitiesPtr.size()) {
                    ...
                    if (entity->getShape().getGlobalBounds().contains(mousePosition.x, mousePosition.y)) {
                        ...
                        if (imageNumbers[i] <= stoi(application->getBddObj().getScore())) {
                            ...
                            std::cout << "Image " << pictureChoose << " cliqu�e" << std::to_string(bdd->getIdPicture()) << std::endl;
            ...
            // V�rifier le clic sur le back button
...
void LockerWindow::UpdatePos(sf::Time deltaTime) {
    ...
    float swayAmountX =  0;//cos(time * swaySpeed) * swayMagnitude; // cos pour commencer au centre
...
void LockerWindow::Render()
{
    ...
    // Dessiner le bouton pr�c�dent uniquement si pas dans la premi�re section
    if (currentSectionIndex > -1) {
        window.draw(buttonPrevSprite);
    }

    // Dessiner le bouton suivant uniquement si pas dans la derni�re section
