...
void TCPServer::bindAndListen()
{
    ...
    if (bind(idSocket, (struct sockaddr*)&adr_server, sizeof(adr_server)) == -1) {
        throw std::runtime_error("Bind Failed");
    }

    if (listen(idSocket, 1) == -1) {
...
void TCPServer::acceptClients()
{
    while (isServerOn) {
        ...
        if (select(idSocket + 1, &readfds, nullptr, nullptr, &timeout) > 0) 
        {
            ...
            int clientSocket = accept(idSocket, (struct sockaddr*)&adr_client, &addr_len);
...
void TCPServer::matchClientsForGame()
{
    while (isServerOn) {
        ...
        int client1 = matchmakingQueue.front(); matchmakingQueue.pop();
        if (!isClientConnected(client1)) continue; 

        int client2 = matchmakingQueue.front(); matchmakingQueue.pop();
...
void TCPServer::gameSession(int client1, int client2, bool isFirstPlayerToPlay)
{
    std::string message = "";
    ...
    try {
        while (isServerOn) {
            ...
            // V�rifier si le client1 s'est d�connect�
...
std::string TCPServer::receiveMessageFromClient(int clientId)
{
...
bool TCPServer::isSocketActive(int clientSocket) {
    char buffer;
...
bool TCPServer::isClientConnected(int clientSocket)
{
